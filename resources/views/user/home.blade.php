    ...
    <div class="position-relative z-1 container text-start">
        <div class="row">
            <div class="col-lg-8">
                ...
                <a href="{{ route('watches.index') }}" class="btn btn-gold">Explore The Collection</a>
            </div>
        </div>
    </div>
</section>
...
<section class="py-5 my-5">
    <div class="container py-5">
        ...
        <div class="row g-4">
            @forelse($featuredWatches as $watch)
            <div class="col-md-4">
                <div class="luxury-card">
                    <div class="img-zoom-container">
                        <img src="{{ $watch->image ? asset('storage/' . $watch->image) : 'https://images.unsplash.com/photo-1523170335258-f5ed11844a49?q=80&w=2680&auto=format&fit=crop' }}" alt="{{ $watch->name }}">
                    </div>
                    
                    <div class="card-body p-5 text-center">
                        <small class="text-gold-flat text-uppercase ls-3 d-block mb-3">{{ $watch->brand }}</small>
                        <h4 class="text-white serif-font mb-4 fs-4">{{ $watch->name }}</h4>
                        
                        <div class="d-flex justify-content-between align-items-center pt-4 border-top border-secondary border-opacity-25">
                            <span class="text-white fs-5" style="font-weight: 300;">${{ number_format($watch->price) }}</span>
                            <a href="{{ route('watches.show', $watch->id) }}" class="text-gold-flat text-decoration-none ls-3 small">DISCOVER &rarr;</a>
                        </div>
                    </div>
                </div>
            </div>
