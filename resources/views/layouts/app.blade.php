                ...
                @else
                    ...
                    <li class="nav-item dropdown position-relative">
                        <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">
                            ...
                            @if($unreadCount > 0)
                                <span class="position-absolute top-0 start-100 translate-middle badge rounded-circle bg-danger text-white" style="font-size: 0.55rem;">{{ $unreadCount }}</span>
                            @endif
                        </a>
                        <ul class="dropdown-menu dropdown-menu-end bg-dark border-secondary shadow-lg py-0" style="width: 280px; font-size: 0.8rem;">
                            <li class="p-3 border-bottom border-secondary border-opacity-25"><h6 class="mb-0 text-gold-flat small ls-2">Notifications</h6></li>
                            @forelse(auth()->user()->unreadNotifications->take(4) as $n)
                            <li class="border-bottom border-secondary border-opacity-25">
                                <form action="{{ route('notifications.read', $n->id) }}" method="POST">
                                    @csrf
                                    <button class="dropdown-item text-light p-3 position-relative d-flex align-items-center" style="white-space: normal;">
                                        @if(isset($n->data['image']))
                                            <img src="{{ asset('storage/' . $n->data['image']) }}" class="rounded me-2 border border-secondary" style="width: 36px; height: 36px; object-fit: cover;">
                                        @endif
                                        <div style="flex: 1;">
                                            <small class="d-block text-gold mb-1" style="font-size: 0.72rem;">{{ $n->data['title'] ?? 'Alert' }}</small>
                                            <span class="small d-block text-white-50">{{ \Illuminate\Support\Str::limit($n->data['message'] ?? '', 40) }}</span>
                                        </div>
                                    </button>
                                </form>
                            </li>
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end bg-dark border-secondary shadow-lg">
                            <li><a class="dropdown-item text-light small ls-3" href="{{ route('wishlist.index') }}">WISHLIST</a></li>
                            <li><a class="dropdown-item text-light small ls-3" href="{{ route('orders.history') }}">ORDERS</a></li>
                            <li><a class="dropdown-item text-light small ls-3" href="{{ route('notifications.settings') }}">ALERTS</a></li>
