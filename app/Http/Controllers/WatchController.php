...
class WatchController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (\App\Models\SiteSetting::getValue('notifications_enabled', '1') === '1') {
            $users = \App\Models\User::where('is_admin', false)->get();
            \Illuminate\Support\Facades\Notification::send($users, new \App\Notifications\NewWatchNotification($watch));
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->price < $oldPrice && \App\Models\SiteSetting::getValue('notifications_enabled', '1') === '1') {
            $wishlists = \App\Models\Wishlist::where('watch_id', $watch->id)->with('user')->get();
            foreach ($wishlists as $wishlist) {
                $wishlist->user->notify(new \App\Notifications\PriceDropNotification($watch, $oldPrice));
            }
        }
